...
const mockLogin = async (email: string, password: string): Promise<User> => {
  ...
  // Mock users for demo
  const adminUser = { id: '1', name: 'Admin User', email: 'admin@electra.com', role: 'admin' as const };
  const voterUser = { id: '2', name: 'Voter User', email: 'voter@electra.com', role: 'voter' as const };
  
  // Check credentials (in a real app, this would be done on the server)
  if (email === adminUser.email && password === 'admin') {
    return adminUser;
  } else if (email === voterUser.email && password === 'voter') {
    return voterUser;
  }
